...
export const NewOrderForm = ({ itemId, itemName, onClose, onComplete }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!itemId || !formData.type || formData.platinum === "" || formData.quantity === "") {
    ...
    const payload = {
      itemId,
      type: formData.type,
      platinum: Number(formData.platinum),
      quantity: Number(formData.quantity),
      visible: Boolean(formData.visible),
    };

    if (formData.rank !== "") {
      payload.rank = Number(formData.rank);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <label className="block">
            ...
            <select
              ...
              className="mt-1 block w-full rounded-md border-gray-300 border p-2 shadow-sm focus:border-green-500 focus:ring-green-500 bg-white"
              required
            >
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">Rank (Optional)</span>
            <input
              type="number"
              name="rank"
              value={formData.rank}
